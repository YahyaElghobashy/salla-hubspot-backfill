...
def main():
    ...
    ap.add_argument("--since", required=True, help='"YYYY-MM-DD HH:MM:SS" (engine local time)')
    ...
    log.info("%s -- %d create(s) in the window", "APPLY" if args.apply else "DRY RUN", len(pairs))
    written = []
    for ts, cid, contact in pairs:
        ...
        if (p.get("salla_customer_id") or "").strip():
            ...
            continue
        try:
            c = relay.get_path(f"customers/{cid}").get("data") or {}
        except Exception as e:
            log.error("  customer %s: relay failed (%s) -- skipped", cid, e)
            continue
        if str(c.get("id") or "") != str(cid):
            log.error("  customer %s: relay returned id %r -- skipped", cid, c.get("id"))
            continue
        phone = f"{c.get('mobile_code', '')}{c.get('mobile', '')}"
